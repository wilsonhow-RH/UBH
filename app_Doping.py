...
def create_unified_plot(fig, cached_data, system_mode, theta_deg, zoom_factor, q_max, view_mode, boundary_mode, mid_panel_mode, den_cmap, den_contrast, relax_mode, w1, w2, user_zmin, user_zmax, k_elastic, k_vdw, eph_g0, eph_decay, is_video_frame=False):
    ...
    if fig is None:
        # Adjusted height to 8.5 to create perfectly square bounding boxes for flush colorbars
    ...
    # Perfectly symmetric margins: left=0.05, right=0.95
    gs = fig.add_gridspec(1, 3, width_ratios=[1, 1, 1], wspace=0.12, left=0.05, right=0.95, bottom=0.1, top=0.88)
    ...
    if mid_panel_mode == 'Geometry (Density)':
        ...
        cbar2 = fig.colorbar(im2, ax=ax2, shrink=0.45, pad=0.03, anchor=(0.0, 0.0))
    ...
    cbar3 = fig.colorbar(im3, ax=ax3, shrink=0.45, pad=0.03, anchor=(0.0, 0.0))
    ...
    # Top-right anchored inside the axes safely above the flush colorbar
    ax3.legend(loc='upper right', fontsize=8, framealpha=0.8, ncol=1, labelspacing=0.8)
