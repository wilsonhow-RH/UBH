...
def create_unified_plot(fig, cached_data, system_mode, theta_deg, zoom_factor, q_max, view_mode, boundary_mode, mid_panel_mode, den_cmap, den_contrast, relax_mode, w1, w2, user_zmin, user_zmax, k_elastic, k_vdw, eph_g0, eph_decay, is_video_frame=False):
    ...
    # NEW: 5-column asymmetrical GridSpec. 
    # Spacer 1 (0.15) > Spacer 2 (0.09) mechanically shifts Panel 2 to the right to balance visual gaps.
    gs = fig.add_gridspec(1, 5, width_ratios=[1, 0.15, 1, 0.09, 1], wspace=0.0, left=0.05, right=0.95, bottom=0.1, top=0.88)
    ...
    # NEW: anchor Y-axis shifted to 0.12 to align with the image's bottom edge, clearing the axes labels
